...
demoApp.controller('${class.name}Controller', function($scope, $location, $routeParams, $uibModal,
	<#list class.properties as property><#if property.association == true && property.upper == -1  && property.next?? && property.next == true>${property.type?uncap_first}Service, </#if></#list>
	<#list class.properties as property><#if property.association == true && property.upper == 1 >${property.name}Service, </#if></#list> 
